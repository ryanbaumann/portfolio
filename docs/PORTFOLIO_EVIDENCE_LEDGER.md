...
| Loop Engineering Coding Agent | `observable` | The package contains one vendor-neutral system prompt, four role overlays, a deterministic structural checker, and 16 specified regression scenarios. | [`agent-scripts/coding-agent-loop/`](../agent-scripts/coding-agent-loop/) | 2026-07-16 | The structural check passes and a correlated read-only review informed corrections. Behavioral trials are not recorded; do not claim a cross-model benchmark. |
