...
| Ryan’s Lab applications | `observable` | Ryan’s Lab combines same-origin reference apps with selected external experiments listed in `apps.json`. | [Public repository](https://github.com/ryanbaumann/Portfolio), live routes from `apps.json` | 2026-07-17 | Distinguish workspace-hosted apps from external destinations. Say `no secrets shipped to the browser`, not `zero secrets`. |
| Portfolio architecture | `observable` | One Cloud Run container serves the portfolio and workspace apps through a zero-dependency Node gateway; external Lab entries are outbound links. The portfolio uses small inline scripts for theme and privacy-limited analytics. | [Public repository](https://github.com/ryanbaumann/Portfolio) | 2026-07-17 | Validate deployed network behavior when making runtime claims about OAuth, cold starts, quotas, or CI/CD. |
